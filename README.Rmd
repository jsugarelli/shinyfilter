...
1. Call `define_filters()` to bind the filters to the columns of the dataframe you are presenting in the `reactable`. The arguments of `define_filters()` are the following:
    ...
    * a named vector of the columns of the dataframe that will be filtered; the names of the vector elements are the `inputId`s of the `selectizeInput` widgets that represent the filters
...
If your tooltips/popovers are not visible attach the `shinyBS` package directly in your Shiny app by adding `library(shinyBS)` to your code. The `shinyBS` package is used to create the tooltips and popovers.
...
![shinyfiler logo](man/figures/shinyfilter_animated.gif)
...
GitHub: [https://github.com/jsugarelli/shinyfiler](https://github.com/jsugarelli/shinyfilter)
